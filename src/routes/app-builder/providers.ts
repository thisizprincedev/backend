 ...
 */
router.post('/:id/test', ...authenticatedOnly, asyncHandler(async (req: Request, res: Response) => {
    const { id } = req.params;
    const userId = req.user!.id;
 ...
 */
router.get('/firebase/config', ...authenticatedOnly, asyncHandler(async (req: Request, res: Response) => {
    const userUuid = req.user!.uuid;
    if (!userUuid) {
        return res.status(400).json({ success: false, error: 'User UUID not resolved. Please re-login.' });
    }
...
router.get('/global-config', ...adminOnly, asyncHandler(async (_req: Request, res: Response) => {
    ...
    if (globalConfigError) {
        logger.error(globalConfigError, 'Global config fetch error:');
...
router.post('/global-config', ...adminOnly, asyncHandler(async (req: Request, res: Response) => {
    const { config: newProviderConfig } = req.body;

    if (!newProviderConfig) {
        return res.status(400).json({ success: false, error: 'Config required' });
    }

    const { error: updateGlobalConfigError } = await supabase
        .from('global_config')
        .upsert({
            config_key: 'app_builder_db_provider_config',
            config_value: newProviderConfig,
            updated_at: new Date().toISOString(),
        }, { onConflict: 'config_key' });

    if (updateGlobalConfigError) {
        logger.error(updateGlobalConfigError, 'Global config update error:');
...
router.get('/universal-firebase', ...adminOnly, asyncHandler(async (_req: Request, res: Response) => {
    const { data: universalFirebaseConfigData, error: universalFirebaseConfigError } = await supabase
        .from('global_config')
        .select('config_value')
        .eq('config_key', 'app_builder_universal_firebase_config')
        .maybeSingle();

    if (universalFirebaseConfigError) {
        logger.error(universalFirebaseConfigError, 'Universal Firebase fetch error:');
        return res.status(500).json({ success: false, error: 'Failed to fetch config' });
    }

    return res.json({
        success: true,
        config: universalFirebaseConfigData?.config_value || {
            databaseUrl: "",
            apiKey: "",
            appId: "",
            projectId: "",
