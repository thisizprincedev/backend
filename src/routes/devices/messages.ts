...
/**
 * GET /api/v1/devices/cloud-messages
 * List Geelark SMS messages with filters
 */
...
/**
 * GET /api/v1/devices/cloud-messages/:id
 * Get single Geelark SMS message
 */
...
/**
 * POST /api/v1/devices/cloud-messages/:id/forward
...
router.post('/:id/forward', ...adminOnly, asyncHandler(async (req: Request, res: Response) => {
    ...
    if (!chatId) {
        return res.status(400).json({ success: false, error: 'Telegram chatId is required' });
    }
...
⏰ ${format(new Date(message.received_at), 'PPpp')}`;
    ...
    return res.json({ success: true, message: 'Message forwarded to Telegram successfully' });
}));

/**
 * DELETE /api/v1/devices/cloud-messages/:id
 * Delete Geelark SMS message
 */
